...
public:
	...
	NormalScene(Camera* mainCam, int newWidth, int newHeight) {
		sceneCam = mainCam;
		...
		// Create and bind a new framebuffer object
		glGenFramebuffers(1, &demoFBO);
		glBindFramebuffer(GL_FRAMEBUFFER, demoFBO);
		// Create a new texture object for the framebuffer's color attachment
		...
		// Setup depth texture
		// Create a new texture object for the framebuffer's depth attachment
	...
	Camera* NormalScene::getEarthSceneCamera() {

		return sceneCam;
	}

	GLuint NormalScene::getEarthSceneTexture() {
